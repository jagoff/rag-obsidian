...
The caller (main Devin) uses your plan to spawn agents in the right order. Keep the plan executable, not philosophical.
...
| `subagent_explore` (built-in) | Open-ended research across the codebase | Edits |
| `subagent_general` (built-in) | Fallback for tasks that don't fit any specialist | Anything that fits a specialist — route there first |

If a task clearly needs a generalist + one specialist (e.g. new CLI subcommand that calls into retrieval), split it: assign one of `developer-{1,2,3}` to scaffold the subcommand, specialist implements the domain logic. When dispatching multiple parallel tasks to generalists, give each a distinct slot (`developer-1` for task A, `developer-2` for task B, etc.) so peer instances can claim them concurrently without collision.

## Invariants you must surface when at risk

Flag these explicitly in the plan's "Risks" section if the work touches them:

- `_COLLECTION_BASE = "obsidian_notes_v11"` — schema bump = full re-index cost.
- URL sub-index `obsidian_urls_v1`.
- Reranker on `device="mps"` + `float32` (CPU fallback = 3× slower; fp16 on MPS causes score collapse).
- Ollama `keep_alive=-1` on every call; `chat_keep_alive()` auto-clamps large models to `20m`.
- Confidence gates: `CONFIDENCE_RERANK_MIN=0.015`, `CONFIDENCE_DEEP_THRESHOLD=0.10`.
- `RERANK_POOL_MAX=15` (post-2026-04-21 bench — don't raise without re-benching).
- Eval floor — verify current numbers against CLAUDE.md "Eval baseline" section before dispatching retrieval work. Compare via overlapping CIs, never bare point estimates.
- Ranker-vivo auto-rollback gate (nightly `com.fer.obsidian-rag-online-tune` 03:30): fails if singles/chains drop below lower CI bounds. Scoring changes WILL trigger this — pre-validate via `rag tune` offline + manual `rag eval`.
- HyDE opt-in only (qwen2.5:3b HyDE drops singles hit@5 ~5pp; re-test on helper change).
- `reformulate_query` MUST stay on HELPER (qwen2.5:3b), not chat.
- Reranker title-prefix `{title}\n({folder})\n\n{parent_body}` — don't strip when refactoring rerank input assembly.
- `RAG_EXPLORE=1` enables ε-exploration on `morning`/`today` plists. MUST be unset during `rag eval` (the command pops + asserts).
...
- Local-first: no new cloud API dependencies (FREE + local rule; Gmail/Calendar are cross-source user override, see CLAUDE.md §10.6).
- Cross-source ingester guard: `_is_cross_source_target(vault_path)` — only default vault receives the 11 ETLs unless opted in.
- Prompt-injection defence: all LLM context chunks must go through `_format_chunk_for_llm` (redaction + fencing centralised).

## Peer coordination

Before producing the plan, check active peers via `mcp__claude-peers__list_peers(scope: "repo")` if available. If peers are active and their `set_summary` overlaps with domains your plan touches:
...
- **Option C**: recommend a git worktree for truly parallel work on `rag.py`.
