...
| `OBSIDIAN_RAG_MOZE_FOLDER` | `04-Archive/99-obsidian-system/99-AI/external-ingest/Finanzas/MOZE` | (ver `rag/integrations`) | Path del vault donde cae la ingesta MOZE. Override si reorganizaste PARA. |
| `OBSIDIAN_RAG_MOZE_DIR` | `~/Library/Mobile Documents/iCloud~amoos~Tally4/Documents` | `rag/cross_source_etls.py:128`, `web/server.py:8792` | Carpeta iCloud donde Tally4 guarda los `MOZE_*.csv`. Separado de `Finances/` desde el 2026-05-04 (antes el dir era compartido con los xlsx de tarjeta). |
| `OBSIDIAN_RAG_FINANCE_DIR` | `~/Library/Mobile Documents/com~apple~CloudDocs/Finances` | `rag/cross_source_etls.py:135`, `web/server.py:8796` | Carpeta iCloud donde el banco deja los `Último resumen*.xlsx` y PDFs de transferencias. |
| `OBSIDIAN_RAG_WATCH_EXCLUDE_FOLDERS` | unset | (ver `rag.watch`) | Lista coma-separada de carpetas que `rag watch` ignora. Útil para mantener `04-Archive/99-obsidian-system/99-AI/external-ingest/WhatsApp` o `04-Archive` fuera del re-index continuo. |
