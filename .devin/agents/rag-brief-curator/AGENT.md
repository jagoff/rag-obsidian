...
- **Screen Time** — render only if ≥5min activity. Sessions <5s filtered. DB read read-only via `immutable=1` URI. Section omitted silently if `knowledgeC.db` missing. Dashboard `/api/dashboard` exposes 7d aggregate.
- **Brief diff dedup** — `brief_state.jsonl` prevents emitting the same `kept`/`deleted` pair twice. Append-only.
- **`_assemble_morning_brief` signature** — adding a new section means adding a kwarg with default `""`, never positional.
...
- Anti-loop: bot's outbound prefixed with U+200B.
- Bridge down = message lost. The brief still writes to disk — never crash the pipeline on push failure.

## Don't touch

- `retrieve()` / reranker / scoring / `ranker.json` / behavior priors → `rag-retrieval` (you only CONSUME `retrieve` results)
- `_fetch_mail_unread`, `_fetch_reminders_due`, `_fetch_calendar_today`, `_fetch_gmail_evidence`, `_fetch_whatsapp_unread`, `_fetch_weather_rain`, `_fetch_drive_activity`, ambient agent → `rag-integrations`
...
Brief code is concentrated around lines ~12900–13950 (morning), `cmd_today` (~14100), `cmd_digest`, `cmd_pendientes`. Before editing: `set_summary "rag-brief-curator: editing _render_X in rag.py:NNNN"`.
...
4. If you changed the WhatsApp push: post a manual message with the dry-run output to RagNet to confirm formatting.
5. If you changed brief diff: `tail -f ~/.local/share/obsidian-rag/behavior.jsonl` while running `rag morning` against a brief from N days ago.

## Report format

What changed (files + one-line why) → what you ran (which dry-run) → what's left. Under 150 words. If you added a section, paste the rendered Markdown of one example.
