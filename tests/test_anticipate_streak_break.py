...
Cubre:
- Sin 99-obsidian/99-AI/reviews dir → []
...
def mock_vault(tmp_path, monkeypatch):
    """Vault vacío + 99-obsidian/99-AI/reviews creado + `_resolve_vault_path` apuntando acá."""
...
def empty_vault(tmp_path, monkeypatch):
    """Vault SIN 99-obsidian/99-AI/reviews (para simular setup inicial)."""
    vault = tmp_path / "vault"
...
def _write_brief(vault: Path, brief_date, body: str = "# morning brief\n\nstub.\n") -> Path:
    """Crea `99-obsidian/99-AI/reviews/{brief_date.isoformat()}.md` con body mínimo."""
    path = vault / "99-obsidian/99-AI/reviews" / f"{brief_date.isoformat()}.md"
    path.write_text(body, encoding="utf-8")
    return path


def _write_evening(vault: Path, brief_date, body: str = "# evening brief\n") -> Path:
    """Crea `99-obsidian/99-AI/reviews/{brief_date.isoformat()}-evening.md` (NO debe contar)."""
    path = vault / "99-obsidian/99-AI/reviews" / f"{brief_date.isoformat()}-evening.md"
    path.write_text(body, encoding="utf-8")
    return path
...
def test_no_reviews_dir_returns_empty(empty_vault):
    """Sin folder 99-obsidian/99-AI/reviews → [] silenciosamente."""
    result = streak_break_signal(_REF_NOW)
    assert result == []


def test_empty_reviews_dir_returns_empty(mock_vault):
    """Con 99-obsidian/99-AI/reviews pero sin briefs → [] (pausa voluntaria)."""
    result = streak_break_signal(_REF_NOW)
    assert result == []
...
def test_non_iso_md_files_ignored(mock_vault):
    ...
    (mock_vault / "99-obsidian/99-AI/reviews" / "notes.md").write_text("random", encoding="utf-8")
    (mock_vault / "99-obsidian/99-AI/reviews" / "2025-W12.md").write_text("weekly", encoding="utf-8")
    (mock_vault / "99-obsidian/99-AI/reviews" / "README.md").write_text("readme", encoding="utf-8")
    result = streak_break_signal(_REF_NOW)
...
def test_invalid_date_in_filename_ignored(mock_vault):
    ...
    (mock_vault / "99-obsidian/99-AI/reviews" / "2025-13-45.md").write_text("bad", encoding="utf-8")
    result = streak_break_signal(_REF_NOW)
    assert result == []
...
def test_find_last_morning_brief_empty_vault(mock_vault):
    """Helper retorna None con 99-obsidian/99-AI/reviews vacío."""
