...
| `RAG_INDEX_SAFE` | `1` | `rag/__init__.py` | Perfil conservador automático para todo `rag index`: embed slices, shard de notas grandes, preflight/memory guard y abort limpio si swap/pressure cruza umbral. `0` desactiva las guardas. |
| `RAG_INDEX_FULL_SAFE` | `1` para `--full` | `rag/__init__.py` | Perfil adicional para full rebuild: salta synthetic questions + GLiNER entities, usa embed slices más chicos y pausa más entre flushes. `0` re-activa enriquecimientos pero mantiene `RAG_INDEX_SAFE`. |
| `RAG_INDEX_EMBED_SLICE_SIZE` | `16` / `8` en full safe | `rag/__init__.py` | Cap externo de textos por llamada a `embed()` dentro del indexer. Evita picos de memoria cuando una nota o URL batch genera muchos chunks. `0` desactiva el slicing. |
| `RAG_INDEX_FILE_CHUNK_SLICE_SIZE` | `128` / `64` en full safe | `rag/__init__.py` | Sharding de una nota grande antes del flush. Evita que un único archivo genere miles de chunks/embeddings en una sola unidad de trabajo. `0` desactiva el sharding. |
| `RAG_INDEX_BATCH_SLEEP_MS` | `50` / `150` en full safe | `rag/__init__.py` | Pausa en ms después de cada `_flush_batch()` durante `rag index`. Ejemplo: `150` → ~5 batches/s máximo, GPU respira entre embeddings. No afecta archivos skippeados por hash. |
...
| `RAG_INDEX_MEMORY_GUARD_INTERVAL_S` | `5` | `rag/__init__.py` | Chequeo síncrono de memory pressure entre slices/flushes. Si cruza threshold o swap, unload in-process + clear cache + pausa. Complementa al watchdog thread. |
| `RAG_INDEX_MEMORY_PRESSURE_SLEEP_S` | `10` | `rag/__init__.py` | Pausa aplicada por el memory guard cuando detecta presión durante indexado. |
| `RAG_INDEX_ABORT_ON_MEMORY_PRESSURE` | `1` | `rag/__init__.py` | Si después del unload/pausa sigue por encima de `RAG_INDEX_ABORT_USED_PCT` o `RAG_INDEX_ABORT_SWAP_GB`, aborta el index antes de colgar la máquina. |
...
| `RAG_START_ABORT_SWAP_GB` | `2.0` | `rag/cli/setup.py` | Umbral de swap usado para saltear catch-up/daemons durante `rag start`. |
