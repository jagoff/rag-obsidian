...
Local RAG over an Obsidian vault. Multi-file under `rag/` package (post-split 2026-05-04): `rag/__init__.py` (60,180 lines) + sub-modules `plists.py` (95 KB) / `cross_source_etls.py` (89 KB) / `postprocess.py` (29 KB) / `archive.py` (16 KB) + `mcp_server.py` (thin wrapper, 662 lines) + `web/` (FastAPI `web/server.py` 20.6k lines + ~7.7k JS/HTML/CSS) + `tests/` (6,031 tests, 395 files).
...
## Estructura post-split (2026-05-04)

`rag/` es un paquete Python, ya NO un single-file. Layout:
- `rag/__init__.py` (60,180 LOC) — core: retrieve, query, chat, indexing, ingester wiring, telemetry, brief composition, CLI Click tree.
- `rag/plists.py` (95 KB) — launchd plist factories + `_services_spec`.
- `rag/cross_source_etls.py` (89 KB) — los 13+ ETLs `_sync_*` (Gmail, Calendar, WhatsApp, Reminders, Contacts, Calls, Safari, etc).
- `rag/archive.py` (16 KB) — archive subsystem (`rag archive`, dead note detection).
- `rag/postprocess.py` (29 KB) — citation repair / critique / NLI grounding / response post-processing.
- `rag/anticipatory.py` — anticipatory agent (3 signals: calendar, echo, commitment).
- `rag/brief_schedule.py` — horario auto-tuning de briefs (morning/today/digest).
- `rag/contradictions_penalty.py` — penalty reranking para notas con contradicciones detectadas.
- `rag/voice_brief.py` — síntesis TTS del morning brief (vía `say` de macOS).
- `rag/whisper.py` — transcripción de audios WhatsApp + learning loop.
- `rag/wa_scheduled.py` — worker de mensajes WhatsApp programados.
- `rag/wa_tasks.py` — extracción de tareas de WhatsApp.
- Otros: `mmr_diversification.py`, `mood.py`, `ocr.py`, `pendientes.py`, `today_correlator.py`, `vault_health.py`, etc.

**Re-export pattern**: `__init__.py` hace `from rag.X import *  # noqa: F401, F403` para preservar 100% compat con código externo (`from rag import _watch_plist`, `from rag import _sync_moze_notes`, etc.). Cualquier nuevo sub-módulo debe seguir este patrón con `__all__` explícito.

**Line numbers en CLAUDE.md**: removidas deliberadamente porque driftan rápido en un codebase bajo refactor. Referencias a funciones usan el nombre completo sin línea (`rag/__init__.py` función `_retrieve_impl`, etc.).
...
**3 señales activas** (todas en `rag/__init__.py` bajo `# ── ANTICIPATORY AGENT ──`):
...
**Catálogo completo**: [docs/env-vars-catalog.md](docs/env-vars-catalog.md) lista las **47 env vars adicionales** que `rag/__init__.py` consulta pero no están documentadas aquí (config defaults, feature flags, infra). Esta sección cubre solo las críticas con rollback paths + historia detallada.
...
- `RAG_EXPAND_MIN_TOKENS` — threshold for the `expand_queries()` short-query perf gate (default `4`, `rag/__init__.py`). Queries shorter than this token count (split by whitespace) skip the qwen2.5:3b paraphrase call (~1-3s saved). Raise to be more aggressive about skipping paraphrase; lower to restore pre-2026-04-21 behaviour (`<= 2` tokens skipped). Web server already forced `multi_query=False` globally (`web/server.py:3648`), this makes CLI caller-by-caller.
- `RAG_CITATION_REPAIR_MAX_BAD` — threshold for the citation-repair perf gate (default `2`, `rag/__init__.py`). When `verify_citations()` returns more than this many invented paths, the repair round-trip is skipped entirely (rationale: heavily hallucinated answers rarely recover under a single-shot repair and the 5-8s non-streaming call dominates latency). Set to `0` to disable citation-repair completely. Applies to both `rag query` (`rag/__init__.py`) and `rag chat` (`rag/__init__.py`) paths.
...
- `OBSIDIAN_RAG_MOZE_DIR` — override iCloud source dir for MOZE CSV exports. Default `~/Library/Mobile Documents/iCloud~amoos~Tally4/Documents` (Tally4 app container). Separated from the credit-cards/PDF dir on 2026-05-04 — antes ambas fuentes vivían en `CloudDocs/Finances`. **El nuevo Tally4 (4.x) NO exporta CSV directo — guarda backups `MOZE_*.zip` con un `moze.realm` (Realm Object Database, formato propietario v22) adentro.** El bridge en [`rag/integrations/tally4_realm.py`](rag/integrations/tally4_realm.py) detecta el zip más nuevo, lo descomprime a tmpdir, corre [`scripts/tally4_realm_to_csv.js`](scripts/tally4_realm_to_csv.js) (Node.js + `realm` npm) para volcar `AHRecord` → CSV con shape MOZE legacy, y deja el CSV en `~/.local/share/obsidian-rag/moze_cache/MOZE_<zip_mtime>.csv`. Idempotente: si la mtime del zip no cambió, no re-extrae. Lazy install: la primera invocación corre `npm install realm` en `~/.local/share/obsidian-rag/realm-extractor/` (~150MB, una vez). Silent-fail si node/npm no están en PATH — el caller cae al glob de CSVs históricos manuales en el dir iCloud (back-compat). Type enum mapping observado en MOZE 4.0: `0=Expense, 1=Income, 3=Receivable, 7=Balance Adjustment`.
- `OBSIDIAN_RAG_FINANCE_DIR` — override iCloud source dir for credit-card xlsx (`Último resumen*.xlsx`) and bank PDFs. Default `~/Library/Mobile Documents/com~apple~CloudDocs/Finances`. Tras el split del 2026-05-04 ya NO incluye los CSV de MOZE.
...
- `RAG_NLI_MODE` — Citation NLI verifier mode: `off` (default) / `mark` (add ` (?)` to unverified sentences) / `strip` (remove unverified). Uses `cross-encoder/nli-deberta-v3-small` (~80 MB). Lazy-load with sticky-fail — verifier silently stays off if model isn't cached locally. **Default `off`**: no overhead, no model load. Pre-condition before enabling: `huggingface-cli download cross-encoder/nli-deberta-v3-small`. See **Citation NLI verifier** section under *Response-quality post-pipeline*.
- `RAG_NLI_THRESHOLD` — Entailment score threshold for sentence verification (default `0.5`, float). Sentence is "verified" when max entailment score across all retrieved chunks exceeds threshold. Raise to reduce false-positives (more sentences marked `(?)`); lower to catch fewer hallucinations. Only relevant when `RAG_NLI_MODE != off`.

Dev/debug toggles (not set in production):

- `RAG_DEBUG=1` — verbose stderr in the local embed path (`rag/__init__.py`).
...
| Citation NLI verifier (opt-in) | `cross-encoder/nli-deberta-v3-small` | ~80 MB, MPS-friendly, lazy-load with sticky-fail. Gate: `RAG_NLI_MODE` (default `off`). Separate singleton from NLI grounding above. |

All ollama calls use `keep_alive=OLLAMA_KEEP_ALIVE` — default `-1` (forever) in code (`rag/__init__.py`) since 2026-04-21; launchd plists use the same value for symmetry. `CHAT_OPTIONS`: `num_ctx=4096, num_predict=384` — don't bump unless prompts grow.
...
`handle_agenda(col, params, limit=20, *, question=None)` in `rag/__init__.py` filters metas by `source ∈ _AGENDA_SOURCES = frozenset({"calendar", "reminders"})` via `normalize_source`, sorts by `created_ts` desc (start_ts for calendar events, creation anchor for reminders). Gracefully handles missing/string/malformed `created_ts` (coerce + try/except → 0.0).
...
Cross-source corpus (Gmail / WhatsApp, user override §10.5 = indexá-todo) means a hostile email or WhatsApp can land in the index and reach the LLM context through a legitimate semantic match. Two passive layers in `rag/__init__.py` (right above `SYSTEM_RULES`):
...
**Citation NLI verifier** (`RAG_NLI_MODE`, default `off`): sentence-level semantic entailment check after the LLM stream is assembled. Implemented in `rag/postprocess.py` (`verify_answer_nli`, `apply_nli_mode`). Uses `cross-encoder/nli-deberta-v3-small` (~80 MB, MPS-friendly) loaded lazily via `_get_citation_nli_model()` with sticky-fail (if model not cached → `_CITATION_NLI_LOAD_FAILED=True`, verifier stays silent forever in that process). Model outputs 3-class scores; entailment column index `_NLI_DEBERTA_ENTAIL_IDX=1`. All (sentence × chunk) pairs batched in **one** `model.predict()` call for latency.

Threshold: `RAG_NLI_THRESHOLD` (default `0.5`). A sentence is "verified" when its max entailment score across all retrieved chunks exceeds the threshold.

Three modes (controlled by `RAG_NLI_MODE` env var):
- `off` — no-op, answer passes raw (default, no UX breakage)
- `mark` — appends ` (?)` suffix to sentences that fail the threshold
- `strip` — removes unverified sentences; falls back to `"No tengo evidencia clara para responder."` if result < 30 chars

Wire-up in `web/server.py` `gen()`: runs after `full = "".join(parts)` (stream assembled), before `done` SSE. When mark/strip changes `full`, emits `nli_correction` SSE event `{text, mode, verified, unverified}`. Silent-fail: any exception → answer passes raw, no crash.

Telemetry fields added to `rag_queries.extra_json` and `done` SSE event: `nli_verified_count`, `nli_unverified_count`, `nli_unverified_sentences` (capped at 5 in log).

Invariants:
- `RAG_NLI_MODE=off` (default) → zero overhead, zero model load. Do NOT set to `mark`/`strip` until model is cached locally (`huggingface-cli download cross-encoder/nli-deberta-v3-small`).
- Model is separate from the mDeBERTa singleton (`RAG_NLI_GROUNDING`) — different purpose (sentence-level vs claim-level), different model, different lazy-load singleton.
- `split_sentences_for_nli()` protects URLs + decimals + abbreviations via placeholder map before splitting.

Tests: [`tests/test_citation_nli.py`](tests/test_citation_nli.py) (31 cases).
...
Subsystems have autodescriptive docstrings in `rag/__init__.py` and dedicated test files. Only contracts/invariants here.

**LLM typo correction** (`_correct_typos_llm`, Quick Win #4, 2026-05-04): pre-embed step inside `expand_queries()` that fixes obvious typos using qwen2.5:3b (HELPER_OPTIONS — deterministic `temperature=0, seed=42`) before the query reaches the embedding + BM25 pipeline. Targets real-world user input errors like "asor" → "Astor", "psicologa" → "psicóloga", "biba" → "BICA". Gate: `RAG_TYPO_CORRECTION` env var (default ON). LRU cache 256 entries (`OrderedDict` + `threading.Lock`). Sanity check: if the corrected form is >1.5× the length of the input, the original is preserved (guards against hallucinations). Silent fail on any exception — returns original query without crash. Runs FIRST in the query pipeline: typo → anaphora → reformulate → expand. State shared with `query()` via `_expand_last_llm_typo_original[0]` / `_expand_last_llm_typo_corrected[0]` (list-as-mutable-container pattern) — logged in `rag_queries.extra_json` as `{llm_typo_corrected: bool, llm_typo_original: str|null, llm_typo_corrected_text: str|null}`. Does NOT replace `maybe_normalize_typos()` (vocab-bounded corrector that runs post-retrieve on borderline-confidence results) — the two are complementary: LLM can fix proper nouns not in corpus vocab, vocab-bounded fixes corpus-known word variants. Tests: `tests/test_typo_correction.py` (13 cases).

**Sessions**: JSON per session in `sessions/<id>.json`. TTL 30d, cap 50 turns, history window 6. IDs validated `^[A-Za-z0-9_.:-]{1,64}$`; invalid → mint fresh. WhatsApp passes `wa:<jid>`. **Selective history summarisation** (`_summarize_conversation_history`, Quick Win #5, 2026-05-04): when `len(history) > 2` (more than 1 completed turn) AND `RAG_HISTORY_SUMMARY != "0"/"false"/"no"/""`  (default ON), `/api/chat` compresses all prior turns (N-1) via qwen2.5:3b + `HELPER_OPTIONS` (temperature=0, seed=42) into a 2-3 sentence summary; the last turn is always kept verbatim. Summary injected as `{"role": "system", "content": "[Resumen de la conversación previa]\n..."}` + last turn. Cached in `rag_conversation_summaries(session_id, history_hash, summary, ts)` — compound PK `(session_id, history_hash)`, 30d retention via `rag maintenance`. Silent-fail to raw concatenation on LLM/SQL error; `_bump_silent_log_counter()` on every error path. Tests: [`tests/test_history_summary.py`](tests/test_history_summary.py) (11 cases).

**Anaphora resolver (Quick Win #1, 2026-05-04)**: dedicated upstream rewriter para queries follow-up tipo `y en Madrid?` / `y para mañana?` / `y la otra opción?`. Vive en `rag/__init__.py` antes de `reformulate_query` y se invoca desde `multi_retrieve()` ANTES de la primera llamada a `retrieve()`. Tres componentes: (1) `_is_anaphoric_query(query, history)` — detector regex-only (microsegundos): True cuando `len(history) >= 1` AND (query <8 tokens OR empieza con conector `y` / `pero` / `también` / `ahora` / `y para` / `y en` / `y de` / `y la` / `y el` / `y los` / `y las` / `y eso` / `y otro` / `y otra`); (2) `_resolve_anaphora(query, history)` — usa `qwen2.5:3b` con `HELPER_OPTIONS` (temp=0, seed=42) y prompt rioplatense que pide expandir SOLO referencias anafóricas usando los últimos 4 turnos del historial; LRU cache 128 entries por `(history_hash, query)`; silent-fail → return original query (mismo contract que `reformulate_query`); clamp defensivo si el helper devuelve un output >3× el largo del input (señal de explicación pegada). (3) Wire-up en `multi_retrieve` reemplaza `question` por la rewrite ANTES del per-vault loop, así un único helper call cubre todos los vaults. Telemetría en `extras` del `RetrieveResult` → `to_log_event` la levanta al `rag_queries.extra_json` con keys `anaphora_resolved` (bool), `anaphora_original` (str), `anaphora_rewritten` (str). Cuando el rewrite coincide con la query original (helper devolvió la misma) marcamos `resolved=False` para no inflar telemetría con falsos positivos. Gate: `RAG_ANAPHORA_RESOLVER` env var (default ON; `0` / `false` / `no` lo apaga). NO modifica `reformulate_query` — corre upstream y deja al downstream el cleanup deterministic. Tests: `tests/test_anaphora_resolver.py` (23 cases — detector, resolver happy path, cache, silent-fail, clamp, gate, integración con `multi_retrieve`, surfacing en `to_log_event`).
...
**Ranker-vivo (closed-loop ranker)**: implicit feedback from daily use re-tunes `ranker.json` nightly without manual intervention. Four signal sources insert into `rag_behavior`: (1) CLI `rag open` wrapper (opt-in via `RAG_TRACK_OPENS=1` + user-registered `x-rag-open://` handler); (2) WhatsApp listener classifying follow-up turns (`/save`, quoted reply → positive; "no"/"la otra"/rephrase → negative; 120s silence → weak positive); (3) web `/api/behavior` POST from home dashboard `sendBeacon` clicks; (4) morning/today brief diff (`_diff_brief_signal` compares yesterday's written brief vs current on-disk — wikilinks that survived = `kept`, missing = `deleted`, dedup via `rag_brief_state`). Nightly `com.fer.obsidian-rag-online-tune` at 03:30 runs `rag tune --online --days 14 --apply --yes`, which calls `_behavior_augmented_cases` (weight=0.5, drops conflicts), backs up current `ranker.json` → `ranker.{ts}.json` (keeps 3 newest), re-tunes, runs the bootstrap-CI gate (`_run_eval_gate`: scrubs `RAG_EXPLORE`, subprocess `rag eval`, 10min cap, regex parses hit@5). If singles < `GATE_SINGLES_HIT5_MIN` (default 0.60, override via `RAG_EVAL_GATE_SINGLES_MIN`) OR chains < `GATE_CHAINS_HIT5_MIN` (default 0.73, override via `RAG_EVAL_GATE_CHAINS_MIN`) → auto-rollback + exit 1 + log to `rag_tune`. `rag tune --rollback` restores the most recent backup manually. **Floor recalibrados 2026-04-23** desde los originales 0.7619 / 0.6364: con la expansión de `queries.yaml` (42→60 singles post-2026-04-21, +cross-source/synthesis/comparison goldens deliberadamente más duros), el baseline estable cayó a 71.67% / 86.67% y los floors fueron rebajados a los nuevos CI lower bounds (mismo criterio metodológico: "95% confianza de que corridas bajo el floor son regresión real, no noise"). Ver el bloque de comentarios sobre `GATE_SINGLES_HIT5_MIN` en `rag/__init__.py` (~línea 23121) para la timeline completa.

**LLM-as-judge rerank (condicional, default OFF — 2026-05-04)**: cuando el cross-encoder no diferencia (`top_score < RAG_LLM_JUDGE_THRESHOLD`, default 0.5) Y `len(candidates) >= RAG_LLM_JUDGE_MIN_CANDIDATES` (default 5) Y `RAG_LLM_JUDGE=1` (master gate), corremos un round-trip al helper LLM (qwen2.5:3b, `HELPER_OPTIONS` deterministic) que scoree top-20 candidates 0-10 y blendea con el cross-encoder vía `α * ce + (1 - α) * (llm_score / 10)` (α default 0.5, override `RAG_LLM_JUDGE_ALPHA`). Vive en [`rag/llm_judge.py`](rag/llm_judge.py), inyectado en `retrieve()` justo después del `reranker.predict()` y antes del score-loop. Apunta a +5-15pp hit@5 en queries duras (~25% del tráfico con top_score bajo) a costa de +1-2s tail latency cuando dispara. **Default OFF** (`RAG_LLM_JUDGE` no seteado): cero overhead, `should_fire_judge()` short-circuits antes de cualquier import. **Skip implícito** (no llegamos al gate aunque flag esté ON): `fast_path_taken=True` (queries fáciles donde sem+BM25 coinciden, scores son sintéticos), `propose_intent` (caller filtra antes), scope estrecho (`source` / `folder` / `path` filter — caller ya decidió pool chica). Cross-encoder fp32 path queda intacto upstream del judge. Silent fail: timeout/connection/JSON parse → log via `_silent_log` + return None → caller deja ranking original (graceful degrade, `parse_failed=True` surface en telemetry). JSON output forzado vía `format="json"` + repair fallback que extrae primer `{...}` sub-string (qwen2.5:3b a veces wrap en ```json``` aunque le pidamos puro). Determinismo: temperature=0, seed=42 → reruns mismo score. Telemetría persistida en `rag_queries.extra_json`: `llm_judge_fired: bool, llm_judge_ms: int, llm_judge_top_score_before: float, llm_judge_top_score_after: float, llm_judge_parse_failed: bool, llm_judge_n_candidates: int`. Tunables: `RAG_LLM_JUDGE_THRESHOLD` (default 0.5), `RAG_LLM_JUDGE_ALPHA` (default 0.5, clamped a [0,1]), `RAG_LLM_JUDGE_POOL` (default 20, capped por num_ctx=4096 del judge call), `RAG_LLM_JUDGE_MIN_CANDIDATES` (default 5), `RAG_LLM_JUDGE_DEBUG=1` (loguea parse failures explícitamente). Tests: [`tests/test_llm_judge_rerank.py`](tests/test_llm_judge_rerank.py) (30 casos cubriendo trigger conditions, blending formula, JSON parse, silent fail, telemetry shape, dataclass legacy compat). **Status**: prototipo shipped 2026-05-04, `rag eval` ON+OFF + latency P95 cuando dispara aún PENDIENTE — operador debe correr antes de promover a default ON.
...
`SOURCE_RECENCY_HALFLIFE_DAYS` es global por source, pero queries time-sensitive ("cuándo es mi próximo turno", "qué hago mañana") necesitan boost MUCHO más fuerte de notas recientes que queries históricas ("qué hablamos el año pasado"). Capa de modifiers ENCIMA del halflife default que respeta la intent temporal de la query — sin tocar el dict global ni los call sites que ya consumen `source_recency_multiplier`.

**Detector** `_query_temporal_intent(query) → "recent" | "historical" | "neutral"` (`rag/__init__.py`, junto a `source_recency_multiplier_with_intent`): regex puro, case-insensitive, español + inglés, determinístico (misma query → mismo intent siempre), sin LLM. Patterns:
- **recent**: `próxim[oa]`, `cuándo`, `mañana`, `hoy`, `ayer`, `esta semana`, `este mes`, `ahora`, `pendientes`, `sin leer`, `recientes`, `últimos`, `siguiente` + EN equivalents (`next`, `upcoming`, `tomorrow`, `today`, `unread`, `latest`, `recently`, `currently`).
- **historical**: `el año pasado`, `mes pasado`, `semana pasada`, `hace X tiempo`, `cuando estaba/era/vivía`, `histórico` + EN (`last year/month/week`, `X years ago`, `back when I was`, `previously`, `in the past`).
- **neutral**: default (no patterns match).

Historical gana sobre recent cuando ambos disparan — pistas de pasado distante son menos ambiguas.

**Modifier** `source_recency_halflife_for_intent(source, intent) → halflife | None`:

| Intent      | Multiplier | WA (60d) | Gmail (180d) | Vault (None) |
|-------------|-----------:|---------:|-------------:|-------------:|
| `recent`    | × 0.3      | 18d      | 54d          | None         |
| `historical`| × 3.0      | 180d     | 540d         | None         |
| `neutral`   | × 1.0      | 60d      | 180d         | None         |

Vault / Calendar / Contacts siguen `None` siempre (atemporales — no decay aunque la query pida lo más reciente).

**Multiplier** `source_recency_multiplier_with_intent(src, ts, intent, *, now=None)` reemplaza al legacy `source_recency_multiplier` en `retrieve()` (un solo call site, post-rerank, dentro del loop de scoring). Para `intent="neutral"` (o `RAG_INTENT_RECENCY=0`) devuelve **bit-idéntico** al legacy. `apply_weighted_scores` queda con el legacy para preservar paridad con el offline-tune del eval gate.

**Wiring**: `retrieve()` computa `_query_temporal_intent(question)` UNA vez al inicio (regex puro, ~1µs) y lo propaga al loop de scoring. `RetrieveResult.temporal_intent` lleva el valor al caller; `to_log_event` lo persiste a `rag_queries.extra_json.temporal_intent` para análisis downstream (distribución per-intent en eval gate, correlación con hit@5 / abandono).

**Gate**: `RAG_INTENT_RECENCY` env var. **Default ON** (`val not in ("0","false","no","off")`). Setear `RAG_INTENT_RECENCY=0` desactiva el feature → multiplicador legacy bit-idéntico (rollback inmediato). Sin env vars adicionales — los multipliers (`0.3 / 3.0 / 1.0`) están hardcoded en `_INTENT_RECENCY_MULTIPLIERS` del módulo. Tunable solo via edit + redeploy si el eval gate justifica calibración.

Tests: [`tests/test_intent_recency.py`](tests/test_intent_recency.py) (55 casos: detector ES+EN 25+ queries, modifier per-source/per-intent, multiplier numeric integration, gate truthy/falsy variants, determinismo, RetrieveResult propagation, retrieve() empty-collection edge).
...
Dos fuentes, un solo ingester: `~/Library/Safari/History.db` (SQLite) + `~/Library/Safari/Bookmarks.plist` (binary plist que incluye Reading List como subtree `com.apple.ReadingList`). Complementa el inline Chrome ETL existente (`rag/__init__.py` (función `_sync_chrome_history`) escribe `.md` al vault) — Safari va por la arquitectura source-prefixed (chunks en la DB vectorial directo, sin polución al vault).
...
- **`telemetry.db`** (~36M) — **45+ tablas** operativas: `rag_queries`, `rag_behavior`, `rag_feedback`, `rag_feedback_golden*`, `rag_tune`, `rag_contradictions`, `rag_ambient*`, `rag_brief_*`, `rag_wa_tasks`, `rag_archive_log`, `rag_filing_log`, `rag_eval_runs`, `rag_surface_log`, `rag_proactive_log`, `rag_cpu_metrics`, `rag_memory_metrics`, `system_memory_metrics`, `rag_conversations_index`, `rag_conversation_summaries` (Quick Win #5, history summarisation cache), `rag_response_cache`, `rag_entities`, `rag_entity_mentions`, `rag_ocr_cache`, `rag_vlm_captions`, `rag_audio_transcripts`, `rag_learned_paraphrases`, `rag_cita_detections`, `rag_score_calibration`, `rag_chunk_contexts` (Contextual Retrieval cache, gated 2026-05-04), `rag_schema_version`.
...
- `rag_conversation_summaries` (Quick Win #5, 2026-05-04) — LLM-compressed summaries of prior conversation turns. PK `(session_id, history_hash)`; 30d retention via `rag maintenance`. Written/read by `_summarize_conversation_history()` in `rag/__init__.py`. Key: SHA-256[:16] of JSON-serialised N-1 messages. Populated by `/api/chat` when `len(history) > 2` AND `RAG_HISTORY_SUMMARY=1` (default).
...
Primitives in `rag/__init__.py` (`# ── SQL state store (T1: foundation) ──` section):
