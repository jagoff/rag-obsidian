...
| `OBSIDIAN_RAG_MOZE_FOLDER` | `99-obsidian/99-AI/external-ingest/Finanzas/MOZE` | (ver `rag/integrations`) | Path del vault donde cae la ingesta MOZE. Override si reorganizaste PARA. |
...
| `OBSIDIAN_RAG_WATCH_EXCLUDE_FOLDERS` | unset | (ver `rag.watch`) | Lista coma-separada de carpetas que `rag watch` ignora. Útil para mantener `99-obsidian/99-AI/external-ingest/WhatsApp` o `04-Archive` fuera del re-index continuo. |
