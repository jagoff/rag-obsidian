...
_PROPOSE_CREATE_OVERRIDE = (
    ...
    "       (reuniones, visitas, cumpleaños, turnos, viajes).\n"
    "     - `propose_reminder(title, when, ...)` — para recordatorios "
    "       (pagos, llamadas, tareas).\n"
    "     Reminder vs Event: si hay HORA DE INICIO concreta → evento. "
    "     'acordate de X' / tarea → reminder. "
    "     Visitas/cumpleaños/viajes = eventos (aunque sean sin hora).\n"
    ...
    "  4. SIN HORA: NO pasés `all_day`. El tool detecta automáticamente "
    "     si el input tiene hora o no y setea all_day=True cuando "
    "     corresponde. El default del tool es correcto para 'el "
    "     miercoles viene Grecia' (all-day) y para 'el miercoles 4pm' "
    "     (timed). Vos solo pasá `start` tal cual.\n"
    "  5. DESPUÉS del tool call (en el turn siguiente del assistant "
    "     sin tool_calls), respondé con UNA sola oración breve "
    "     confirmando. Variá la redacción según el caso.\n\n"
    "EXCEPCIÓN — preguntar SIN llamar el tool: sólo si la pregunta es "
    "COMPLETAMENTE ambigua (ej. 'recordame algo', 'creá un evento' "
    "sin más info). En ese caso respondé con 1 pregunta corta. "
    "NO preguntes por una hora 'que falta' — si no hay hora, el tool "
    "crea all-day, eso es válido.\n\n"
    "Si NO llamás el tool (y no es el caso de excepción), el usuario "
    "no ve nada creado. La respuesta de texto SIN tool call en el "
    "caso normal es un bug."
)
