...
def mock_vault(tmp_path, monkeypatch):
    """Vault vacío + 05-Reviews creado + `_resolve_vault_path` apuntando acá."""
...
def empty_vault(tmp_path, monkeypatch):
    """Vault SIN 05-Reviews (para simular setup inicial)."""
    vault = tmp_path / "vault"
...
def _write_brief(vault: Path, brief_date, body: str = "# morning brief\n\nstub.\n") -> Path:
    """Crea `05-Reviews/{brief_date.isoformat()}.md` con body mínimo."""
    path = vault / "05-Reviews" / f"{brief_date.isoformat()}.md"
    path.write_text(body, encoding="utf-8")
    return path


def _write_evening(vault: Path, brief_date, body: str = "# evening brief\n") -> Path:
    """Crea `05-Reviews/{brief_date.isoformat()}-evening.md` (NO debe contar)."""
    path = vault / "05-Reviews" / f"{brief_date.isoformat()}-evening.md"
    path.write_text(body, encoding="utf-8")
    return path
...
def test_non_iso_md_files_ignored(mock_vault):
    ...
    (mock_vault / "05-Reviews" / "notes.md").write_text("random", encoding="utf-8")
    (mock_vault / "05-Reviews" / "2025-W12.md").write_text("weekly", encoding="utf-8")
    (mock_vault / "05-Reviews" / "README.md").write_text("readme", encoding="utf-8")
    result = streak_break_signal(_REF_NOW)
...
def test_invalid_date_in_filename_ignored(mock_vault):
    ...
    (mock_vault / "05-Reviews" / "2025-13-45.md").write_text("bad", encoding="utf-8")
    result = streak_break_signal(_REF_NOW)
    assert result == []
...
def test_find_last_morning_brief_empty_vault(mock_vault):
    """Helper retorna None con 05-Reviews vacío."""
