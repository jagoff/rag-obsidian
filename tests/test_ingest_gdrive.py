...
(user's Obsidian workflow was moving `99-obsidian/99-AI/external-ingest/GoogleDrive/` to
...
Tests cover:
  ...
  9. `run()` end-to-end with a `FakeCollection` + `FakeService`:
     ...
     ``99-obsidian/99-AI/external-ingest/GoogleDrive/`` so vault-side leftovers can't leak
...
# ── 10. invariant: is_excluded skips 99-obsidian/99-AI/external-ingest/GoogleDrive/ ─────────────
...
def test_is_excluded_blocks_leftover_gdrive_folder_by_default():
    ...
    skip `99-obsidian/99-AI/external-ingest/GoogleDrive/` so any stale markdown (daily snapshot,
    legacy backfill) never gets vault-side double-indexed alongside the
    real `source="drive"` chunks."""
    assert rag.is_excluded("99-obsidian/99-AI/external-ingest/GoogleDrive/archive/foo.md") is True
    assert rag.is_excluded("99-obsidian/99-AI/external-ingest/GoogleDrive/2026-04-24.md") is True
