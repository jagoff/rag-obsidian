...
- `RAG_WA_FAST_PATH` / `RAG_WA_FAST_PATH_THRESHOLD` — extiende el fast-path de `RAG_ADAPTIVE_ROUTING` a queries dominadas por WhatsApp. El default gate exige `top_score > _LOOKUP_THRESHOLD` (0.6) — queries conversacionales suelen scorear en [0.3, 0.6] porque el ruido del chat dampea la confianza del cross-encoder → caen al pipeline full (qwen2.5:7b num_ctx grande + citation repair). Nuevo gate: `RAG_ADAPTIVE_ROUTING` ON + (source filter explícito `"whatsapp"` OR ≥2 de los top-3 metas tienen `source="whatsapp"`) + top-1 score > `RAG_WA_FAST_PATH_THRESHOLD` (default 0.3). La majority check protege contra un chunk WA incidental disparando el fast-path. **Default ON** (`RAG_WA_FAST_PATH != "0"`). Rollback: `export RAG_WA_FAST_PATH=0` o subir el threshold a 1.0+. Medido 2026-04-22: P50 queries con top-1 WA fue 10.38s / P95 66.69s — el fast-path reduce el gen a qwen2.5:3b num_ctx=4096 + skip citation-repair (~5-8s ahorrados en gen). Tests: [`tests/test_wa_perf_fast_path.py`](tests/test_wa_perf_fast_path.py) (7 casos sobre trigger por majority, explicit source, rollback env, threshold override, sin adaptive routing).
...
- `rag_response_cache` — semantic response cache (GC#1, 2026-04-22). Key shape: `(id, ts, question, q_embedding BLOB, dim, corpus_hash, intent, ttl_seconds, response, paths_json, scores_json, top_score, hit_count, last_hit_ts, extra_json)`. Lookup: cosine ≥ `_SEMANTIC_CACHE_COSINE` (default 0.97 vía `RAG_CACHE_COSINE`) contra las últimas `_SEMANTIC_CACHE_MAX_ROWS` entradas del mismo `corpus_hash` dentro de `ttl_seconds`; hits bump `hit_count` + `last_hit_ts`. **Writes on hot path corren en background (2026-04-22)**: `query()` llama `semantic_cache_store(background=True)` que encola el INSERT en el daemon `rag-bg-sql-writer` y retorna inmediatamente. Pre-fix cada store bloqueaba el hilo principal hasta 1.3s bajo WAL contention (retry budget 5×~0.25s); audit 2026-04-22 encontró ~314 `semantic_cache_store_failed` entries clustered en bursts de <1 min (concurrent web traffic + CLI + tests), cada uno robando ~1.3s al tiempo de respuesta percibido. Con background mode el hilo principal devuelve la respuesta al usuario y el daemon absorbe los retries off-thread. **Gates de store** (aplican igual en sync y background): (a) cache disabled, (b) corpus_hash vacío, (c) response vacío, (d) `top_score < 0.015` (refuse por gate de confianza), (e) `_is_refusal(response)` matchea (refuse conceptual del LLM — patrón añadido 2026-04-22 tras observar cache poisoning: una query con top_score 1.18 cacheó "No tengo esa información" y envenenó queries similares permanentemente). Helpers: `semantic_cache_lookup()` / `semantic_cache_store()` / `semantic_cache_clear(corpus_hash?)` / `semantic_cache_stats()`. Tests: [`tests/test_semantic_cache.py`](tests/test_semantic_cache.py) (22 casos cubriendo cosine threshold, TTL expiry, corpus hash invalidation, dim mismatch, hit_count bumps, refusal gating, concurrent stores + el batch nuevo de background: enqueue + commit, gating respeta, no bloquea bajo contention simulada 500ms).
