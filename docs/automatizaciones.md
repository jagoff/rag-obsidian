...
### morning (7am lun-vie)
Genera un archivo nuevo en `tu-vault/00-Inbox/reviews/YYYY-MM-DD.md`. Si tu Mac estuvo apagado a esa hora, no corre (no hay "catch-up"). Lo podés correr a mano: `rag morning`.

### today (22pm lun-vie)
Genera `tu-vault/00-Inbox/reviews/YYYY-MM-DD-evening.md`. Si no hubo actividad en el día, imprime "sin actividad hoy" y no escribe nada.

### digest (domingo 22pm)
Genera `tu-vault/00-Inbox/reviews/YYYY-WNN.md` (semana ISO). Es un brief narrativo largo.

### archive (día 1 de cada mes, 23pm)
Corre `rag archive --apply --gate 20` — si detecta más de 20 notas a archivar, no lo aplica sin `--force`. Escribe un reporte a `00-Inbox/reviews/YYYY-MM-archive.md`.
