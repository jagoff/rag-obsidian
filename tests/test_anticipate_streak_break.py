...
def mock_vault(tmp_path, monkeypatch):
    """Vault vacío + 00-Inbox/reviews creado + `_resolve_vault_path` apuntando acá."""
...
def empty_vault(tmp_path, monkeypatch):
    """Vault SIN 00-Inbox/reviews (para simular setup inicial)."""
    vault = tmp_path / "vault"
...
def _write_brief(vault: Path, brief_date, body: str = "# morning brief\n\nstub.\n") -> Path:
    """Crea `00-Inbox/reviews/{brief_date.isoformat()}.md` con body mínimo."""
    path = vault / "00-Inbox/reviews" / f"{brief_date.isoformat()}.md"
    path.write_text(body, encoding="utf-8")
    return path


def _write_evening(vault: Path, brief_date, body: str = "# evening brief\n") -> Path:
    """Crea `00-Inbox/reviews/{brief_date.isoformat()}-evening.md` (NO debe contar)."""
    path = vault / "00-Inbox/reviews" / f"{brief_date.isoformat()}-evening.md"
    path.write_text(body, encoding="utf-8")
    return path
...
def test_no_reviews_dir_returns_empty(empty_vault):
    """Sin folder 00-Inbox/reviews → [] silenciosamente."""
    result = streak_break_signal(_REF_NOW)
    assert result == []


def test_empty_reviews_dir_returns_empty(mock_vault):
    """Con 00-Inbox/reviews pero sin briefs → [] (pausa voluntaria)."""
    result = streak_break_signal(_REF_NOW)
    assert result == []
...
def test_non_iso_md_files_ignored(mock_vault):
    ...
    (mock_vault / "00-Inbox/reviews" / "notes.md").write_text("random", encoding="utf-8")
    (mock_vault / "00-Inbox/reviews" / "2025-W12.md").write_text("weekly", encoding="utf-8")
    (mock_vault / "00-Inbox/reviews" / "README.md").write_text("readme", encoding="utf-8")
    result = streak_break_signal(_REF_NOW)
...
def test_invalid_date_in_filename_ignored(mock_vault):
    ...
    (mock_vault / "00-Inbox/reviews" / "2025-13-45.md").write_text("bad", encoding="utf-8")
    result = streak_break_signal(_REF_NOW)
    assert result == []
...
def test_find_last_morning_brief_empty_vault(mock_vault):
    """Helper retorna None con 00-Inbox/reviews vacío."""
